...
object ApplicationBuild extends Build {
  ...
  val moduleCommon = play.Project(
    appName + "-common", appVersion, appDepsCommon, path = file("modules/common")).settings(
  ...
  val moduleCp = play.Project(
    appName + "-cp", appVersion, appDepsCp, path = file("modules/cp")).dependsOn(
      moduleCommon).aggregate(
        moduleCommon).settings(
          // Disable generating scaladoc
          sources in doc in Compile := List(),
  ...
  val modulePplogin = play.Project(
    appName + "-pplogin", appVersion, appDepsPplogin, path = file("modules/pplogin")).dependsOn(
      moduleCommon).aggregate(
        moduleCommon).settings(
          // Disable generating scaladoc
          sources in doc in Compile := List(),
  ...
  val modulePayCharging = play.Project(
    appName + "-paycharging", appVersion, appDepsPplogin, path = file("modules/paycharging")).dependsOn(
      moduleCommon).aggregate(
        moduleCommon).settings(
          // Disable generating scaladoc
          sources in doc in Compile := List(),
  ...
  val main = play.Project(appName, appVersion, appDependenciesBase, path = file(".")).settings(
    ...
    // Force compilation in java 1.6
    javacOptions in Compile ++= Seq("-source", _javaVersion, "-target", _javaVersion)).dependsOn(
      moduleCommon, moduleCp, modulePplogin, modulePayCharging).aggregate(
        moduleCommon, moduleCp, modulePplogin, modulePayCharging)
}
